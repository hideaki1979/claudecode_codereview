# GitHub Actions 自動ラベル付与の設定ガイド

このドキュメントでは、PR 分析と自動ラベル付与の GitHub Actions を設定する手順を説明します。

## 📋 機能概要

PR が作成・更新されるたびに自動的に:

- PR 差分を分析
- リスクレベルを評価
- 適切なラベルを自動付与
- 分析結果をコメントで通知

## 🏷️ 自動付与されるラベル

### リスクレベル

- 🟢 `low-risk` - リスクレベル: 低
- 🟡 `medium-risk` - リスクレベル: 中
- 🔴 `high-risk` - リスクレベル: 高
- 🚨 `critical-risk` - リスクレベル: 緊急

### 特徴フラグ

- 📊 `large-changes` - 500 行以上の大規模変更
- ⚠️ `critical-files-modified` - `package.json`, `.env` などの設定ファイル変更（詳細はスクリプトを参照）
- 🔒 `security-review-needed` - セキュリティレビュー推奨（将来実装）

## 🚀 セットアップ手順

### 1. リポジトリ権限の確認

GitHub Actions がラベルを付与できるように、リポジトリ設定を確認します。

1. リポジトリの **Settings** → **Actions** → **General** を開く
2. **Workflow permissions** セクションで以下を確認:
   - ✅ "Read and write permissions" を選択
   - ✅ "Allow GitHub Actions to create and approve pull requests" にチェック

### 2. ファイルをプッシュ

以下のファイルがリポジトリに含まれていることを確認:

```text
.github/
├── workflows/
│   └── pr-analysis.yml       # ワークフロー定義
├── scripts/
│   └── analyze-and-label.js  # 分析・ラベル付与スクリプト
└── ACTIONS_SETUP.md          # このファイル
```

これらのファイルを main ブランチにプッシュします:

```bash
git add .github/
git commit -m "feat: GitHub Actions自動ラベル付与を追加"
git push origin main
```

### 3. 動作確認

テスト用の PR を作成して動作を確認:

1. 新しいブランチを作成

   ```bash
   git checkout -b test/auto-labeling
   ```

2. 何かファイルを変更

   ```bash
   echo "# Test" >> test.md
   git add test.md
   git commit -m "test: GitHub Actions動作確認"
   git push origin test/auto-labeling
   ```

3. GitHub で PR を作成

4. **Actions** タブで "PR Analysis & Auto Labeling" ワークフローの実行を確認

5. PR ページでラベルが自動付与されているか確認

6. 分析結果のコメントが投稿されているか確認

## 🔧 カスタマイズ

### ラベルの追加・変更

`.github/scripts/analyze-and-label.js` の `LABELS` オブジェクトを編集:

```javascript
const LABELS = {
  risk: {
    // ... 既存のラベル ...
  },
  features: {
    // 新しいラベルを追加
    customLabel: {
      name: "custom-label",
      color: "0052CC",
      description: "カスタムラベル",
    },
  },
};
```

### 分析ロジックのカスタマイズ

`analyzeSimplified()` 関数を編集して独自の分析ロジックを実装できます。

本格的な分析が必要な場合は、`src/lib/analysis` の既存ロジックを利用することも可能です。

### トリガーイベントの変更

`.github/workflows/pr-analysis.yml` の `on` セクションを編集:

```yaml
on:
  pull_request:
    types: [opened, synchronize, reopened, ready_for_review]
```

## 🐛 トラブルシューティング

### ラベルが付与されない

1. **権限を確認**:

   - Settings → Actions → General → Workflow permissions が "Read and write permissions" になっているか

2. **ワークフローの実行ログを確認**:

   - Actions タブで該当のワークフロー実行を開く
   - エラーメッセージを確認

3. **GITHUB_TOKEN の権限を確認**:
   - ワークフローファイルの `permissions` セクションを確認

### 分析結果が不正確

- `.github/scripts/analyze-and-label.js` の `analyzeSimplified()` 関数を確認
- 必要に応じて閾値を調整

### コメントが投稿されない

- `analysis-result.json` が正しく生成されているか確認
- Actions ログで "Comment Analysis Results" ステップのエラーを確認

## 📚 参考リンク

- [GitHub Actions Documentation](https://docs.github.com/en/actions)
- [Octokit REST API](https://octokit.github.io/rest.js/)
- [GitHub Actions: permissions](https://docs.github.com/en/actions/using-jobs/assigning-permissions-to-jobs)

## 💡 今後の拡張案

- [ ] セキュリティ脆弱性スキャンの統合
- [ ] テストカバレッジの評価
- [ ] AI（Claude/GPT-4）によるコードレビュー提案
- [ ] Slack/Discord 通知
- [ ] 分析結果のトレンド追跡
